...
// default sorting callback
    ...
    Array.include({
         ...
         */
        walk: function () {
            this.map.apply(this, arguments).forEach(function (value, i) {
                this[i] = value;
            }, this);
            return this;
        },
...
// hooking up the class-methods to the root class
    ...
    var Observer = RightJS.Observer = new Class({
             ...
             */
            on: function () {
                Observer_on(this, arguments, function (h) {
                    return h;
                });
                return this;
            },
            ...
            observes: function (event, callback) {
                ...
                if (isString(callback)) {
                    callback = callback in this ? this[callback] : null;
                }
        ...
        if (typeof element === 'string') {
            ...
            if (options !== undefined) {
                for (var key in options) {
                    switch (key) {
                        case 'id':
                            inst._.id = options[key];
                            break;
                        case 'html':
                            inst._.innerHTML = options[key];
                            break;
                        case 'class':
                            inst._.className = options[key];
                            break;
                        case 'on':
                            inst.on(options[key]);
                            break;
                        default:
                            inst.set(key, options[key]);
                    }
                }
            }
        } else {
 ...
 *   });
    ...
    var Xhr = RightJS.Xhr = new Class(Observer, {
        ...
        update: function (element, params) {
            return this.onSuccess(function (r) {
                element.update(r.text);
            }).send(params);
        },
...
// protected
        ...
        stateChanged: function () {
            ...
            if (xhr.readyState != 4 || xhr.canceled) {
                return;
            }
        ...
        initCallbacks: function () {
            ...
            Xhr.EVENTS.each(function (name) {
                this.on(name, function () {
                    Xhr.fire(name, this, this.xhr);
                });
            }, this);
        },

        showSpinner: function () {
            Xhr.showSpinner.call(this, this);
        },
        hideSpinner: function () {
            Xhr.hideSpinner.call(this, this);
        }
    });
...
// attaching the common spinner handling
    $ext(Observer_create(Xhr), {
        ...
        trySpinner: function (context, method) {
            ...
            if (spinner) {
                spinner[method](object.spinnerFx, {duration: 100});
            }
        },
    ...
    var Fx = RightJS.Fx = new Class(Observer, {
             ...
             */
            start: function () {
                if (fx_add_to_queue(this, arguments)) {
                    return this;
                }
    ...
    function clean_styles(element, before, after) {
        ...
        for (key in after) {
            ...
            if (after[key] !== before[key] && /color/i.test(key)) {
                ...
                if (!is_transparent(after[key])) {
                    after[key] = after[key].toRgb();
                }
                if (!is_transparent(before[key])) {
                    before[key] = before[key].toRgb();
                }
        ...
        prepare: function (start, end) {
            ...
            if (is_transparent(end_color)) {
                this.onFinish(function () {
                    element_style[style_name] = 'transparent';
                });
...
// protected
    ...
    Fx.Scroll = new Class(Fx.Attr, {
        ...
        prepare: function (value) {
            ...
            if ('x' in value) {
                attrs.scrollLeft = value.x;
            }
            if ('y' in value) {
                attrs.scrollTop = value.y;
            }
