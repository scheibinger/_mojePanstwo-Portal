...
var Localizer = Class.create({
    ...
    request_position_notAvailable: function () {
        positionNotAvailable = new Element('div', {className: 'positionNotAvailable'}).insert('<p>' + _mPHeart.translation.LC_KTO_TU_RZADZI_POSITION_NOT_AVAILABLE + '</p>');
    ...
    success_callback: function (position) {
        ...
        this.localizerObj.coordinates.update('<p><span class="label">' + _mPHeart.translation.LC_KTO_TU_RZADZI_POSITION_LAT + ':</span> <span class="value">' + position.coords.latitude + '&deg;</span></p><p><span class="label">' + _mPHeart.translation.LC_KTO_TU_RZADZI_POSITION_LAN + ':</span> <span class="value">' + position.coords.longitude + '&deg;</span></p>').appear({duration: .3});

        jQuery.get('/kto_tu_rzadzi/geo/resolve/' + position.coords.latitude + '/' + position.coords.longitude + '.json', function (data) {
            ...
            if (that.gminy.length) {
                ...
                var h = new Element('h2').update(_mPHeart.translation.LC_KTO_TU_RZADZI_POSITION_GMINA + ':');
                that.localizerObj.content.insert(that.gminy_div.insert(h).insert(that.gminy_ul));

                that.gminy_div.appear({duration: .3});

            } else that.localizerObj.display.update(_mPHeart.translation.LC_KTO_TU_RZADZI_POSITION_CANNOT_GMINA);
        });
    },

    /*RETURN ERRORS FORM LOCACTION SYSTEM*/
    error_callback: function (error) {
        var strMessage = _mPHeart.translation.LC_KTO_TU_RZADZI_POSITION_CANNOT_POSITION;
        switch (error.code) {

            case error.PERMISSION_DENIED:
                strMessage = _mPHeart.translation.LC_KTO_TU_RZADZI_POSITION_CANNOT_BROWSER;
                break;

            case error.POSITION_UNAVAILABLE:
                strMessage = _mPHeart.translation.LC_KTO_TU_RZADZI_POSITION_CANNOT_TEMPORARY;
                break;

            case error.TIMEOUT:
                strMessage = _mPHeart.translation.LC_KTO_TU_RZADZI_POSITION_CANNOT_LIMIT;
                break;
...
var PLBrowser = Class.create({
        options: {
            ...
            'center': [51.592656737404596, 19.57128906250001],
            'zoom': 6,
            'minZoom': 6,
            'scrollOffset': 33
        },
        ...
        add_level_after_finish: function (layer) {
            ...
            new Effect.Tween(null, 0, 500, {duration: .3, afterFinish: function (map_canvas, layer) {
        ...
        add_level_get_data: function (layer, map_info) {
            ...
            switch (Number(layer.l)) {
                case 1:
                    ajaxUrl = '/kto_tu_rzadzi/geo/wojewodztwa';
                    break;

                case 2:
                    ajaxUrl = '/kto_tu_rzadzi/geo/powiaty';
                    break;

                case 3:
                    ajaxUrl = '/kto_tu_rzadzi/geo/gminy';
                    break;
...
jQuery(document).ready(function () {
    var plBrowser = new Localizer(),
        localizer = new PLBrowser();
});