    ...
    "brush": {
        ...
        "extent": {
            ...
            "returns a two-dimensional array if x and y are defined": function (brush) {
                var b = brush().x(_.scale.linear()).y(_.scale.linear());
                assert.deepEqual(b.extent(), [
                    [0, 0],
                    [0, 0]
                ]);
            },
            "takes a two-dimensional array if x and y are defined": function (brush) {
                var b = brush().x(_.scale.linear()).y(_.scale.linear()).extent([
                    [0.1, 0.2],
                    [0.3, 0.4]
                ]);
                assert.deepEqual(b.extent(), [
        ...
        "empty": {
            "returns true if and only if any defined extent is empty": function (brush) {
                ...
                assert.strictEqual(b.extent([
                    [0, 0],
                    [1, 0]
                ]).empty(), true); // x is non-empty, y is empty
                assert.strictEqual(b.extent([
                    [0, 0],
                    [0, 1]
                ]).empty(), true); // x is empty, y is non-empty
                assert.strictEqual(b.extent([
                    [0, 0],
                    [1, 1]
                ]).empty(), false); // x is non-empty, y is non-empty
            }
        }
    }
});
