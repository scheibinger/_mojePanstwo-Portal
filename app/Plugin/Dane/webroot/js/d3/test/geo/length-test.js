...
suite.addBatch({
    "length": {
        ...
        "the length of points are zero": function (length) {
            ...
            assert.inDelta(length({type: "MultiPoint", coordinates: [
                [0, 1],
                [2, 3]
            ]}), 0, 1e-6);
        },
        "the length of a line string is the sum of its great arc segments": function (length) {
            assert.inDelta(length({type: "LineString", coordinates: [
                [-45, 0],
                [45, 0]
            ]}), Math.PI / 2, 1e-6);
            assert.inDelta(length({type: "LineString", coordinates: [
            ...
            assert.inDelta(length({type: "MultiLineString", coordinates: [
                [
                    [-45, 0],
                    [-30, 0]
                ],
        ...
        "the length of a polygon is its perimeter": function (length) {
            assert.inDelta(length({type: "Polygon", coordinates: [
                [
                    [0, 0],
                    [3, 0],
                    [3, 3],
            ...
            assert.inDelta(length({type: "MultiPolygon", coordinates: [
                [
                    [
                        [0, 0],
                        [3, 0],
