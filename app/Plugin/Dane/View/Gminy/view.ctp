		            	...
		            	<div id="organizacje" class="block nobottomborder">
				            ...
				            <div class="content">
				                ...
				                <? foreach( $organizacje as $organizacja ) {?>
				                	<li class="list-group-item"><? if( $organizacja->getData('wartosc_kapital_zakladowy') ) {?><span class="badge"><?= number_format_h($organizacja->getData('wartosc_kapital_zakladowy')) ?></span><? } ?><a href="<?= $organizacja->getUrl() ?>"><?= $this->Text->truncate($organizacja->getShortTitle(), 25); ?></a></li>
				                <? } ?>
				                </ul>
